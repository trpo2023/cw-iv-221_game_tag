...
void shuffle(int board[SIZE][SIZE]) {
  for (int i = SIZE * SIZE - 1; i > 0; i--) {
    ...
    int row_i =
        i / SIZE; // Получаем номер строки, где находится элемент с индексом i
    int col_i =
        i % SIZE; // Получаем номер столбца, где находится элемент с индексом i

    // Преобразование индекса j в строку и столбец
    int row_j =
        j / SIZE; // Получаем номер строки, где находится элемент с индексом j
    int col_j =
        j % SIZE; // Получаем номер столбца, где находится элемент с индексом j
...
void displayMoves(int moves) { printf("Количество ходов: %d\n", moves); }
